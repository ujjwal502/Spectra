...
/**
 * AzureAIService - Azure OpenAI-backed implementation mirroring AIService API.
 *
 * Environment variables expected:
 * - AZURE_OPENAI_API_KEY
 * - AZURE_OPENAI_ENDPOINT (e.g., https://your-resource-name.openai.azure.com)
 * - AZURE_OPENAI_API_VERSION (e.g., 2024-02-15-preview)
 * - AZURE_OPENAI_DEPLOYMENT (default chat/completions deployment name)
 * - AZURE_OPENAI_LARGE_DEPLOYMENT (optional, large-context deployment name)
 */
...
export class AzureAIService {
  ...
  private largeChatModel!: AzureChatOpenAI;
  private embeddingsModel!: AzureOpenAIEmbeddings;
  private config: AzureOpenAIConfig;
  private openai: any; // shim to preserve existing call sites
  ...
  constructor(config?: Partial<AzureOpenAIConfig>) {
    ...
    // Deployment names used as logical model identifiers to keep existing logic
    this.defaultModel = this.config.azureOpenAIApiDeploymentName || 'azure-deployment';
    this.largeContextModel =
      process.env.AZURE_OPENAI_API_LARGE_DEPLOYMENT_NAME || this.defaultModel;
    this.defaultTimeout = parseInt(process.env.OPENAI_TIMEOUT || '30000', 10);

    this.isO4Model =
      (this.defaultModel || '').includes('o4') ||
      (this.largeContextModel || '').includes('o4') ||
      (this.defaultModel || '').includes('gpt-4o') ||
      (this.largeContextModel || '').includes('gpt-4o');

    // Shim: preserve old this.openai.chat.completions.create signature
    this.openai = {
      chat: {
        completions: {
          create: async ({ model, messages, max_tokens, temperature }: any) => {
            const useLarge = model === this.largeContextModel;
            const content = await this.invokeAzureChat(messages, useLarge, max_tokens, temperature);
  ...
  private initializeModels(): void {
    ...
    const customFetch = (url: string, opts: any = {}) => {
      ...
      return (fetch as any)(url, { ...opts, agent: customAgent });
    };
    ...
    this.chatModel = new AzureChatOpenAI({
      ...
      configuration: {
        ...
        fetchOptions: { agent: customAgent },
      },
    });

    // Large-context chat model (falls back to default if not provided)
    const largeDeployment =
      process.env.AZURE_OPENAI_API_LARGE_DEPLOYMENT_NAME ||
      this.config.azureOpenAIApiDeploymentName;
    this.largeChatModel = new AzureChatOpenAI({
      azureOpenAIApiKey: this.config.azureOpenAIApiKey,
      azureOpenAIApiInstanceName: this.config.azureOpenAIApiInstanceName,
      azureOpenAIApiDeploymentName: largeDeployment,
      azureOpenAIApiVersion: this.config.azureOpenAIApiVersion,
      azureOpenAIBasePath: this.config.azureOpenAIBasePath,
      temperature: 0.1,
      maxTokens: 4000,
    ...
    // Embeddings (optional; not used by existing logic but available)
  ...
  private async invokeAzureChat(
    messages: Array<{ role: string; content: string }>,
    useLarge: boolean,
    maxTokens?: number,
    temperature?: number,
  ): Promise<string> {
    const model = useLarge ? this.largeChatModel : this.chatModel;
    const tuples: Array<[string, string]> = [];
    for (const m of messages || []) {
      const role = m.role === 'system' ? 'system' : m.role === 'user' ? 'human' : 'human';
      tuples.push([role, String(m.content || '')]);
    }
    const tempModel =
      temperature !== undefined || maxTokens !== undefined
        ? new AzureChatOpenAI({
            azureOpenAIApiKey: this.config.azureOpenAIApiKey,
            azureOpenAIApiInstanceName: this.config.azureOpenAIApiInstanceName,
            azureOpenAIApiDeploymentName: (useLarge
              ? process.env.AZURE_OPENAI_API_LARGE_DEPLOYMENT_NAME ||
                this.config.azureOpenAIApiDeploymentName
              : this.config.azureOpenAIApiDeploymentName) as string,
            azureOpenAIApiVersion: this.config.azureOpenAIApiVersion,
            azureOpenAIBasePath: this.config.azureOpenAIBasePath,
            temperature: temperature ?? 0.1,
            maxTokens: maxTokens ?? (useLarge ? 4000 : 2000),
            maxRetries: 3,
            timeout: useLarge ? 90000 : 60000,
            openAIApiKey: this.config.openaiApiKey,
          })
        : model;
    const resp = await tempModel.invoke(tuples);
    return typeof resp.content === 'string' ? resp.content : JSON.stringify(resp.content);
  }

  /**
   * Expose chat model (default) for callers expecting a LangChain ChatOpenAI-compatible instance
   */
  ...
  /**
   * Expose embeddings model for optional callers
   */
  ...
  /**
   * Simple chat completion wrapper for services that previously used OpenAI SDK.
   */
  async chatCompletion(
    messages: Array<{ role: string; content: string }>,
    opts?: { useLarge?: boolean; maxTokens?: number; temperature?: number },
  ): Promise<string> {
    const useLarge = !!opts?.useLarge;
    const content = await this.invokeAzureChat(
      messages,
      useLarge,
      opts?.maxTokens,
      opts?.temperature,
