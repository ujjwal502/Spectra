...
function writeJsonReport(state: TestingState, outBaseDir: string): void {
  ...
  fs.writeFileSync(
    path.join(reportsDir, 'spectra-test-report.json'),
...
function writeDetailedResults(state: TestingState, outBaseDir: string): void {
  ...
  fs.writeFileSync(
    path.join(reportsDir, 'detailed-test-results.json'),
...
function writeTestCases(state: TestingState, outBaseDir: string): void {
  ...
  const testCases = {
    ...
    testCasesByType: state.testScenarios.reduce(
      (acc, s) => {
        (acc as any)[s.type] = [...((acc as any)[s.type] || []), s];
        return acc;
      },
      {} as Record<string, TestScenario[]>,
    ),
    testCasesByEndpoint: state.testScenarios.reduce(
      (acc, s) => {
        const k = `${s.method} ${s.endpoint}`;
        (acc as any)[k] = [...((acc as any)[k] || []), s];
        return acc;
      },
      {} as Record<string, TestScenario[]>,
    ),
    allTestCases: state.testScenarios.map((scenario) => ({
      ...
      executionStatus:
        (state.testResults.find((r) => r.scenarioId === scenario.id)?.success ?? null) === null
          ? 'not_executed'
          : state.testResults.find((r) => r.scenarioId === scenario.id)?.success
            ? 'passed'
            : 'failed',
    })),
    summary: {
      byType: state.testScenarios.reduce(
        (acc, s) => {
          (acc as any)[s.type] = ((acc as any)[s.type] || 0) + 1;
          return acc;
        },
        {} as Record<string, number>,
      ),
      byEndpoint: state.testScenarios.reduce(
        (acc, s) => {
          const k = `${s.method} ${s.endpoint}`;
...
function generateTestCasesHtml(testScenarios: TestScenario[], testResults: TestResult[]): string {
  ...
  const testsByType = testScenarios.reduce(
    (acc, s) => {
      (acc as any)[s.type] = [...((acc as any)[s.type] || []), s];
      return acc;
    },
  ...
  return Object.entries(testsByType)
    .map(
      ([type, scenarios]) => `
...
function writeHtmlDashboard(state: TestingState, outBaseDir: string): void {
  ...
  const resultsByType = state.testResults.reduce(
    (acc, r) => {
      ...
      (acc as any)[t].total++;
      if (r.success) (acc as any)[t].passed++;
      else (acc as any)[t].failed++;
      return acc;
    },
...
<body>
	<div class="container">
        ...
        .map(
          ([t, st]: any) =>
            `<tr><td>${t.toUpperCase()}</td><td>${st.passed}</td><td>${st.failed}</td><td>${st.total}</td><td>${Math.round((st.passed / st.total) * 100)}%</td></tr>`,
        )
        .join('')}
			</tbody></table>
		</div>
