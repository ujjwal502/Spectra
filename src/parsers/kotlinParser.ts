...
export class KotlinParser {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (
        ...
        const fullPath = classPath
          ? path
            ? `${classPath}${path.startsWith('/') ? path : '/' + path}`
            : classPath
          : path;
   ...
   */
  private extractWebClientEndpoints(
    content: string,
    endpoints: EndpointInfo[],
  ...
  private extractDataClasses(lines: string[], filePath: string, models: ModelInfo[]): void {
    for (let i = 0; i < lines.length; i++) {
      ...
      // Look for class comment/documentation before the class declaration
      ...
      while (
        ...
        if (lines[k].trim().startsWith('//')) {
          classDescription = lines[k].trim().substring(2).trim() + ' ' + classDescription;
        } else if (
          lines[k].trim().startsWith('*') &&
          !lines[k].trim().startsWith('*/') &&
          !lines[k].trim().startsWith('/**')
        ) {
          classDescription = lines[k].trim().substring(1).trim() + ' ' + classDescription;
        }
        k--;
      }
      ...
      if (dataClassMatch) {
        ...
        if (dataClassMatch[2] === '(') {
          ...
          if (propsMatch && propsMatch[1]) {
            // Split by commas but handle nested types with generics properly
            const params = this.splitConstructorParams(propsMatch[1]);

            for (const param of params) {
              const paramParts = param.split(':').map((p) => p.trim());
              if (paramParts.length < 2) continue;

              const propName = paramParts[0].replace(/^val\s+|^var\s+/, '').trim();
              let propType = paramParts[1].split('=')[0].trim();

              // Check for nullability and extract default value if present
              const isNullable = propType.includes('?');
              const defaultValueMatch = param.match(/=\s*(.+)$/);
              const hasDefaultValue = defaultValueMatch !== null;

              // Check for documentation comment above property in constructor
              let propDescription = '';
              const propCommentMatch = constructorContent.match(
                new RegExp(`/\\*\\*\\s*([^*]*(?:\\*(?!/)[^*]*)*)\\*/\\s*${propName}\\s*:`),
              );
              if (propCommentMatch && propCommentMatch[1]) {
                propDescription = propCommentMatch[1].replace(/\*/g, '').trim();
              }

              // If no comment found, use the property name as the description
              if (!propDescription) {
                propDescription = `${propName} property of ${className}`;
              }
              ...
              properties.push({
                ...
                format: this.mapKotlinTypeToFormat(propType),
                required: !isNullable && !hasDefaultValue,
                enum: this.extractEnumValues(propType, lines),
              });
            }
          }
        }
        ...
        models.push({
          name: className,
          description: classDescription || `Data class for ${className}`,
          properties,
        });
      }
    }
  ...
  /**
   * Split constructor parameters, handling nested generics correctly
   *
   * @param constructorContent The constructor parameter string
   * @returns Array of individual parameters
   */
  private splitConstructorParams(constructorContent: string): string[] {
    const params: string[] = [];
    let currentParam = '';
    let nestedLevel = 0;

    for (let i = 0; i < constructorContent.length; i++) {
      const char = constructorContent[i];
  ...
  /**
   * Map Kotlin types to appropriate JSON schema formats
   *
   * @param kotlinType Kotlin type name
   * @returns JSON schema format string or undefined
   */
  private mapKotlinTypeToFormat(kotlinType: string): string | undefined {
    const cleanType = kotlinType.replace('?', '').trim();

    if (cleanType.startsWith('Int')) {
      return 'int32';
    } else if (cleanType.startsWith('Long')) {
      return 'int64';
    } else if (cleanType.startsWith('Float')) {
      return 'float';
    } else if (cleanType.startsWith('Double')) {
      return 'double';
    } else if (cleanType.startsWith('LocalDate')) {
      return 'date';
    } else if (cleanType.startsWith('LocalDateTime') || cleanType.startsWith('ZonedDateTime')) {
      return 'date-time';
    } else if (cleanType.startsWith('UUID')) {
      return 'uuid';
    } else if (cleanType.startsWith('BigDecimal')) {
      return 'decimal';
    } else if (cleanType.startsWith('Email') || cleanType.endsWith('Email')) {
      return 'email';
    } else if (cleanType.startsWith('URL') || cleanType.endsWith('Url')) {
      return 'uri';
    }
  ...
  /**
   * Extract possible enum values for a type if it's an enum reference
   *
   * @param typeName The type name to check
   * @param allLines All file lines to search for the enum
   * @returns Array of enum values or undefined
   */
  private extractEnumValues(typeName: string, allLines: string[]): string[] | undefined {
    // Clean up the type name
    const cleanType = typeName.replace('?', '').trim();

    // Search for enum definition in the content
    let enumStartFound = false;
    ...
    for (let i = 0; i < allLines.length; i++) {
      ...
      if (!enumStartFound) {
        // Look for enum class declaration
        if (line.match(new RegExp(`enum\\s+class\\s+${cleanType}\\b`))) {
          enumStartFound = true;
        }
      } else {
        // Inside enum definition, look for values
        if (line.includes('{')) {
          // Skip the opening line with '{'
          continue;
        } else if (line.includes('}')) {
          // End of enum
          break;
        } else if (line.trim() && !line.startsWith('//')) {
          // Extract enum value (assuming it's the first word/value before any comma or comment)
          const enumValue = line.split(',')[0].split('(')[0].trim();
          if (enumValue) {
      ...
      if (
        ...
        while (j < lines.length && braceCount > 0) {
          ...
          if (currentLine.match(/(?:val|var)\s+[a-zA-Z0-9_]+\s*:/)) {
            const propMatch = currentLine.match(
              /(?:val|var)\s+([a-zA-Z0-9_]+)\s*:\s*([^=]+)(?:=.*)?/,
            );
            if (propMatch) {
  ...
  private mapKotlinTypeToJsonType(kotlinType: string): string {
    ...
    if (
      cleanType.startsWith('Int') ||
      cleanType.startsWith('Long') ||
      cleanType.startsWith('Double') ||
      cleanType.startsWith('Float') ||
      cleanType.startsWith('Short') ||
      cleanType.startsWith('Byte') ||
      ...
      cleanType.startsWith('Collection<')
    ) {
      return 'array';
    } else if (
      cleanType.startsWith('Map<') ||
      cleanType.startsWith('HashMap<') ||
