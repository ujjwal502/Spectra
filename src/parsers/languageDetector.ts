...
export class LanguageDetector {
  ...
  private async hasOpenApiFiles(rootDir: string): Promise<boolean> {
    ...
    for (const file of yamlFiles.slice(0, 5)) { // Check first 5 files only
      try {
    ...
    for (const file of jsonFiles.slice(0, 5)) { // Check first 5 files only
      try {
        const content = fs.readFileSync(file, 'utf8');
        if (content.includes('"openapi":') || content.includes('"swagger":')) {
          return true;
  ...
  private async detectKotlinFramework(rootDir: string): Promise<string> {
    ...
    const kotlinFiles = this.findFiles(rootDir, '.kt');
    for (const file of kotlinFiles.slice(0, 10)) { // Check first 10 files only
      try {
        const content = fs.readFileSync(file, 'utf8');
  ...
  private async isJavaProject(rootDir: string): Promise<boolean> {
    ...
    return fs.existsSync(pomXmlPath) || fs.existsSync(buildGradlePath) || fs.existsSync(gradlePath);
  }
  ...
  private async detectJavaFramework(rootDir: string): Promise<string> {
    ...
    if (fs.existsSync(pomXmlPath)) {
      ...
      if (pomXml.includes('org.jboss.resteasy')) return 'jaxrs';
    }
  ...
  private async detectPythonFramework(rootDir: string): Promise<string> {
    ...
    try {
      ...
      for (const filePath of pythonFiles.slice(0, 20)) { // Check first 20 files only
